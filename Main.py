...
class Pharmacy:
    def __init__(self, root):
        ...
        self.sref_no_var=StringVar()
        self.cus_name_var= StringVar()
        ...
        add_button = Button(down_buttonframe, text="Add Medicine", command=self.addmedicine, font=(
            "arial", 12, "bold"), width=14, fg="black", bg="white", bd=3, relief=RIDGE, activebackground="white", activeforeground="white")
        add_button.grid(row=0, column=0)

        update_button = Button(down_buttonframe, command=self.update_new, text="Update", font=(
            "arial", 13, "bold"), width=14, fg="black", bg="white", bd=3, relief=RIDGE, activebackground="white", activeforeground="white")
        update_button.grid(row=0, column=1)

        delete_button = Button(down_buttonframe, command=self.Delete_medinfo, text="Delete", font=("arial", 13, "bold"), width=13,
                               fg="black", bg="white", bd=3, relief=RIDGE, activebackground="white", activeforeground="white")
        ...
        exit_button = Button(down_buttonframe, command=self.root.quit, text="Exit", font=(
            "arial", 13, "bold"), width=10, fg="black", bg="white", bd=3, relief=RIDGE, activebackground="white", activeforeground="white")
        ...
        # self.search_combo["values"] = ("Select Options", "Ref No.","Name ", "Salt")
        self.search_combo["values"] = ("Select Options", "Ref No.")
        self.search_combo.grid(row=0, column=6)
        self.search_combo.current(0)

        entry_button = Entry(down_buttonframe, font=("times new roman", 15, "bold"), fg="black", bg="grey", bd=3, width=12, relief=RIDGE, textvariable=self.search_txt)
        entry_button.grid(row=0, column=7)


        search_button = Button(down_buttonframe, text="Search", font=("arial", 13, "bold"), width=10, fg="black", bg="white",
                               bd=3, relief=RIDGE, activebackground="white", activeforeground="white", command=self.search_data)
        search_button.grid(row=0, column=8)

        show_button = Button(down_buttonframe, text="Show All", font=("arial", 13, "bold"), fg="black", bg="white",
                             width=10, bd=3, relief=RIDGE, activebackground="white", activeforeground="white", command=self.fetch_new)
        show_button.grid(row=0, column=9)

        sale_btn = Button(down_buttonframe, command=self.sell_medicine, text="Sale", font=(
            "arial", 13, "bold"), width=10, fg="black", bg="white", bd=3, relief=RIDGE, activebackground="white", activeforeground="white")
        sale_btn.grid(row=0, column=10)

        ######## left small frame #######
        left_smallframe = LabelFrame(topframe, bg='grey', bd=10, relief=RIDGE,padx=20, text="Medicine Information", font=("arial", 13, "bold"), fg="black")
        ...
        def validate_integer(input_value):
            try:
                ...
                messagebox.showerror("Error","You have to enter integer")
                return False
        ...
        # self.ref_entry = Entry(left_smallframe, textvariable=self.refno_var, width=24, font=("times new roman", 13, "bold"), fg="black", bg="white")
        # self.ref_entry.grid(row=0, column=1)

        self.ref_entry = Entry(left_smallframe, textvariable=self.refno_var, width=24, font=("times new roman", 13, "bold"), fg="black", bg="white", validate="key")
        self.ref_entry['validatecommand'] = (self.ref_entry.register(validate_integer), '%P')
        self.ref_entry.grid(row=0, column=1)
        ...
        self.medname_entry = Entry(left_smallframe, textvariable=self.medicine_var, width=24, font=("times new roman", 13, "bold"), fg="black", bg="white")
        ...
        self.lot_entry = Entry(left_smallframe, textvariable=self.lotno_var, width=24, font=("times new roman", 13, "bold"), fg="black", bg="white", validate="key")
        self.lot_entry['validatecommand'] = (self.ref_entry.register(validate_integer), '%P')
        self.lot_entry.grid(row=4, column=1)
      
        # 6
        issue_label = Label(left_smallframe, text="Issue Date:\n(DD/MM/YYYY)", padx=2, pady=4, font=(
            "times new roman", 13, "bold"), bg="grey")
        issue_label.grid(row=5, column=0)

        def validate_issdate():
            entered_date = self.issue_entry.get_date()
            today_date = datetime.now().date()
            
            try:
                # Convert dates to datetime objects
                entered_datetime = datetime.strptime(str(entered_date), '%Y-%m-%d').date()
                today_datetime = datetime.strptime(str(today_date), '%Y-%m-%d').date()
                
                # Check if entered date is in the future
                if entered_datetime > today_datetime:
                    messagebox.showerror("Error", "Date cannot be in the future.")
                    self.issuedt_var.set(today_date)
                    return False
        ...
        self.issue_entry = DateEntry(left_smallframe, width=22, font=("times new roman", 13, "bold"), fg="black", bg="white", textvariable=self.issuedt_var, date_pattern="yyyy-mm-dd", datevar=self.issuedt_var)
        # self.issue_entry = Entry(left_smallframe, width=22, font=("times new roman", 13, "bold"), fg="black", bg="white", textvariable=self.issuedt_var)
        self.issue_entry.grid(row=5, column=1)
        self.issue_entry.bind("<FocusOut>", lambda event: validate_issdate())


        # issue_label = Label(left_smallframe, text="Issue Date :\n(MM/DD/YYYY)", padx=2, pady=4, font=(
        #     "times new roman", 13, "bold"), bg="grey")
        # issue_label.grid(row=5, column=0)
        ...
        # 7
        exp_label = Label(left_smallframe, text=" Expiry Date :\n(MM/DD/YYYY)", padx=2, pady=4, font=(
            "times new roman", 13, "bold"), bg="grey")
        exp_label.grid(row=6, column=0)
        
        def validate_expdate():
            entered_date = self.exp_entry.get_date()
            today_date = datetime.now().date()
            ...
            try:
                ...
                if entered_datetime < today_datetime:
                    ...
                    self.expdt_var.set(today_date)
                    return False
                
                # Check if date is in valid format
                # if entered_date != entered_datetime.strftime('%Y-%m-%d'):
                #     raise ValueError
        ...
        self.exp_entry = DateEntry(left_smallframe, width=22, font=("times new roman", 13, "bold"), fg="black", bg="white", textvariable=self.expdt_var, date_pattern="yyyy-mm-dd", datevar=self.expdt_var)
        # self.exp_entry = Entry(left_smallframe, width=22, font=("times new roman", 13, "bold"), fg="black", bg="white", textvariable=self.expdt_var)
        self.exp_entry.grid(row=6, column=1)
        self.exp_entry.bind("<FocusOut>", lambda event: validate_expdate())
        ...
        # self.exp_entry = DateEntry(left_smallframe, selectmode= 'day' , textvariable=self.expdt_var, width=22, font=(
        #     "times new roman", 13, "bold"), fg="black", bg="white")
        # self.exp_entry.grid(row=6, column=1)
        ...
        self.price_entry = Entry(left_smallframe, textvariable=self.price_var, width=28, font=("times new roman", 13, "bold"), fg="black", bg="white", validate="key")
        self.price_entry['validatecommand'] = (self.price_entry.register(validate_integer), '%P')
        self.price_entry.grid(row=1, column=3)
        ...
        self.qt_entry = Entry(left_smallframe, textvariable=self.quantity_var, width=28, font=("times new roman", 13, "bold"), fg="black", bg="white", validate="key")
        self.qt_entry['validatecommand'] = (self.qt_entry.register(validate_integer), '%P')
        self.qt_entry.grid(row=2, column=3)

        #14

        salt_label = Label(left_smallframe, text=" Salt :", padx=2, pady=4, font=(
            "times new roman", 13, "bold"), bg="grey")
        salt_label.grid(row=3, column=2)


        def validate_input(input_value):
                try:
                    re.match("^[a-zA-Z ]*$", input_value)
                    return True
                except ValueError:
                    messagebox.showerror("Error","You have to enter character")
                    return False

        self.salt_entry = Entry(left_smallframe, textvariable=self.salt_var, width=28, font=("times new roman", 13, "bold"), fg="black", bg="white", validate="key")

        self.salt_entry['validatecommand'] = (self.salt_entry.register(validate_input), '%P')
        self.salt_entry.grid(row=3, column=3)



        # self.salt_entry = Entry(left_smallframe, textvariable=self.salt_var, width=28, font=(
        #     "times new roman", 13, "bold"), fg="black", bg="white")
        # self.salt_entry.grid(row=3, column=3)
        ...
        ############ right frame #########
        right_frame = LabelFrame(topframe, bg='grey', bd=10, relief=RIDGE, padx=5,text="New Medicine Add department", font=("arial", 13, "bold"), fg="black")
        ...
        self.no_entry = Entry(right_frame, textvariable=self.ref_variable, width=16, font=("times new roman", 11, "bold"), fg="black", bg="white", validate="key")
        self.no_entry['validatecommand'] = (self.price_entry.register(validate_integer), '%P')
        # self.no_entry.grid(row=2, column=3)
        ...
        add_button = Button(newframe, text="Add", font=("arial", 13, "bold"), width=13, fg="black", bg="white",
                            bd=3, command=self.AddMed, relief=RIDGE, activebackground="white", activeforeground="white")
        add_button.grid(row=0, column=0)

        updatenew_button = Button(newframe, text="Update", font=("arial", 13, "bold"), width=13, fg="black", bg="white",
                                  bd=3, command=self.Update_med, relief=RIDGE, activebackground="white", activeforeground="white")
        updatenew_button.grid(row=1, column=0)

        delnew_button = Button(newframe, text="Delete", font=("arial", 13, "bold"), width=13, fg="black", bg="white",
                               bd=3, relief=RIDGE, activebackground="white", activeforeground="white", command=self.Delete_med)
        ...
        self.info_table = ttk.Treeview(scroll_frame, column=("ref no", "comp name", "type", "medi name", "salt","lot no", "issue", "exp",
        ...
        self.info_table.heading("salt", text="Salt")
        ...
        self.info_table.column("salt", width=100)
    ...
    def addmedicine(self):
        if self.refno_var.get() == "" or self.lotno_var.get() == "" or self.typemed_var.get() == "":
            ...
            new_cursor.execute("Insert into Information(REF_NO,COMPANY_NAME,TYPE_OF_MED,MED_NAME,SALT,LOT_NO,ISSUE_DT,EXP_DT,USES,SIDE_EFFECT,PRECAUTION,DOSAGE,PRICE,QUANTITY) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",(
    ...
    # def sell_medicine(self, medicine_var, quantity_var):
    def sell_medicine(self):

        root = Tk()
        root.title("Sell Medicine ")
        root.geometry("820x350+0+0")
        root.resizable(False, False)

        # topframe = Frame(root, bg='white', bd=10, relief=RIDGE, padx=20)
        # topframe.place(x=0, y=62, width=1350, height=400)



        sell_medFrame = LabelFrame(root, bg='grey', bd=10, relief=RIDGE,padx=20, text="Sell Medicine", font=("arial", 13, "bold"), fg="black")
        sell_medFrame.place(x=0, y=5, width=820, height=350)

        # date_label = Label(sell_medFrame, text= update_label, padx=2, pady=4, font=("times new roman", 13, "bold"), bg="grey")
        # date_label.pack()
        # # date_label.grid(row=0, column=0, sticky=W)

        # # Function to update the date and time Label every second
        # def update_label():
        #     now = datetime.now()
        ...
        #1
        sref_no = Label(sell_medFrame, text="Reference no.:", padx=2, pady=4, font=("times new roman", 13, "bold"), bg="grey")
        sref_no.grid(row=0, column=0, sticky=W)

        sref_no_entry = Entry(sell_medFrame, textvariable=self.sref_no_var, width=24, font=("times new roman", 13, "bold"), fg="black", bg="white")
        sref_no_entry.grid(row=0, column=1)

        #2
        cus_name = Label(sell_medFrame, text="Customer Name:", padx=2, pady=4, font=("times new roman", 13, "bold"), bg="grey")
        cus_name.grid(row=1, column=0, sticky=W)

        cus_name_entry = Entry(sell_medFrame, textvariable=self.cus_name_var, width=24, font=("times new roman", 13, "bold"), fg="black", bg="white")
        cus_name_entry.grid(row=1, column=1)
        
        #3
        med_name = Label(sell_medFrame, text="Medicine Name:", padx=2, pady=4, font=("times new roman", 13, "bold"), bg="grey")
        med_name.grid(row=2, column=0, sticky=W)

        med_name_entry = Entry(sell_medFrame, textvariable=self.med_name_var, width=24, font=("times new roman", 13, "bold"), fg="black", bg="white")
        med_name_entry.grid(row=2, column=1)
        

        #4
        qty = Label(sell_medFrame, text="Quantity Needed:", padx=2, pady=4, font=("times new roman", 13, "bold"), bg="grey")
        qty.grid(row=3, column=0, sticky=W)

        qty_entry = Entry(sell_medFrame, textvariable=self.qty_var, width=24, font=("times new roman", 13, "bold"), fg="black", bg="white")
        qty_entry.grid(row=3, column=1)

        #5 btn
        sell_btn = Button(sell_medFrame, text="Sell", command=self.sell, font=(
            "arial", 12, "bold"), width=14, fg="black", bg="white", bd=3, relief=RIDGE, activebackground="white", activeforeground="white")
        sell_btn.grid(row=4, column=0,padx=2, pady=4)

        # 6 btn
        clear_btn = Button(sell_medFrame, text="Clear", command=self.clear_sale, font=(
            "arial", 12, "bold"), width=14, fg="black", bg="white", bd=3, relief=RIDGE, activebackground="white", activeforeground="white")
        clear_btn.grid(row=4, column=1,padx=2, pady=4)

        #7 btn
        sell_btn = Button(sell_medFrame, text="Exit", command=root.destroy, font=(
            "arial", 12, "bold"), width=14, fg="black", bg="white", bd=3, relief=RIDGE, activebackground="white", activeforeground="white")
        sell_btn.grid(row=4, column=3,padx=2, pady=4)
    ...
    def sell(self):
        try:
            ...
            if self.sref_no_var.get() == "" or self.cus_name_var.get() == "" or self.med_name_var.get() == "" or self.qty_var.get() == "":
                messagebox.showerror("Error","All fields are required")
            elif self.sref_no_var.get() != self.refno_var:
                messagebox.showerror("Error","NOT AVAILABLE OUT OF STOCK !")
            elif self.medicine_var.get() != self.med_name_var:
                messagebox.showerror("Error","MEDICINE NOT FOUND !")
            elif int(self.qty_var.get()) > int(self.quantity_var):
                messagebox.showerror("Error","OUT OF STOCK!")
            else:
                conn=sqlite3.connect(database=r'pharmacy.db')
                new_cursor=conn.cursor()
                add_sale=conn.cursor()

                self.new_qty=int(self.quantity_var)-int(self.qty_var.get())
                # new_cursor.execute("UPDATE Information SET COMPANY_NAME=?, TYPE_OF_MED=?, MED_NAME=?, SALT=?, LOT_NO=?, ISSUE_DT=?, EXP_DT=?, USES=?, SIDE_EFFECT=?, PRECAUTION=?, DOSAGE=?, PRICE=?, QUANTITY=? WHERE REF_NO=?", (
                new_cursor.execute("UPDATE Information SET  QUANTITY=? WHERE REF_NO=?", (
                    self.quantity_var.set(self.new_qty)
                ))
                add_sale.execute("INSERT INTO sale(CUS_NAME, MED_NAME, QTY_NEED) VALUES (?, ?, ?)", (
                    self.cus_name_var.get(),
    ...
    #     try:
    #         conn=sqlite3.connect(database=r'pharmacy.db')
    #         if self.sref_no_var.get() == "" or self.cus_name_var.get() == "" :
    #             if self.med_name_var.get() == "" or self.qty_var.get() == "":
    #                 messagebox.showerror("Error","All fields are required")
    #             else:
    #                 if self.sref_no_var.get() != self.refno_var:
    #                     messagebox.showerror("Error","NOT AVAILABLE OUT OF STOCK !")
    #                 else:
    #                     if self.medicine_var.get() != self.med_name_var.get():
    #                         messagebox.showerror("Error","MEDICINE NOT AVAILABLE !")
    #                     else:
    #                         if int(self.qty_var.get()) >= int(self.quantity_var):
    #                             messagebox.showerror("Error","OUT OF STOCK!")
    ...
    def clear_sale(self):
        self.sref_no_var.set(" ")
        self.cus_name_var.set(" ")
    ...
    def get_cursor(self,event=""):
        ...
        self.salt_var.set(row[4])
        self.lotno_var.set(row[5])
        self.issuedt_var.set(row[6])
        self.expdt_var.set(row[7])
        self.uses_var.set(row[8])
        self.sideeffect_var.set(row[9])
        self.warning_var.set(row[10])
        self.dosage_var.set(row[11])
        self.price_var.set(row[12])
        self.quantity_var.set(row[13])
    ...
    def search_data(self):
        ...
        if selected == "Select Options":
            ...
            # new_cursor.execute("Select * from Information where ?=?",(self.search_by.get(), self.search_txt.get(),))
